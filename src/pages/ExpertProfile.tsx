    ...
    <div className="min-h-screen transition-colors duration-300" style={{ background: t.bg }}>
      ...
      <main className="max-w-xl mx-auto px-6 py-10">
        ...
        <div className="rounded-2xl border p-5 mb-4 space-y-4 transition-colors duration-300"
          ...
          <Field label="Специализация" value={profile.specialization || ''} onChange={v => set('specialization', v)} placeholder="Социальные проекты, НКО..." inputCls={inputCls} inputStyle={inputStyle} labelColor={t.textMuted} />
          <Field label="Место работы" value={profile.workplace || ''} onChange={v => set('workplace', v)} placeholder="ООО Пример, Москва" inputCls={inputCls} inputStyle={inputStyle} labelColor={t.textMuted} />
          <Field label="Должность" value={profile.position || ''} onChange={v => set('position', v)} placeholder="Консультант по грантам" inputCls={inputCls} inputStyle={inputStyle} labelColor={t.textMuted} />
        </div>
